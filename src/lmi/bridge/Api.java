...
import lmi.lifecycle.Pathfinder;
import lmi.runtime.WaitManager;
import static lmi.core.Constant.D_EAST;
import static lmi.core.Constant.RETRY_MAX;
...
import static lmi.core.Constant.WindowTitle.*;
...
public class Api {
  ...
  static void planObject(String planName) {
    ...
    while (!lmi.bridge.MapViewBridge.mapView().isPlanningObject())
...
//        if (ErrorMessageManager.message() != null)
    ...
    if (lmi.bridge.MapViewBridge.mapView().isPlanningObject())
      Interaction.place(LocalPlayer.position(), D_EAST, 3, 0);
  ...
  /// - Throws: ER_INTERRUPTED
  ///     - ER_FAIL
  public static void moveNorth() { AtomicAction.go(LocalPlayer.position().north()); }
  public static void moveEast() { AtomicAction.go(LocalPlayer.position().east()); }
  public static void moveWest() { AtomicAction.go(LocalPlayer.position().west()); }
  public static void moveSouth() { AtomicAction.go(LocalPlayer.position().south()); }
  public static void moveCenter() { AtomicAction.go(LocalPlayer.position().center()); }
  ...
  public static Gob closestGobIn(Array<Gob> gobArray) {
    ...
    for (Gob gob : gobArray) {
      final double distance = LocalPlayer.distance(gob);
