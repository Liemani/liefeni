...
public final class Pathfinder {
  ...
  private static Coord _calculateMapOrigin() {
    final Coord targetLocation = LocalPlayer.position().assignSubtract(TILE_IN_COORD * CHUNK_SIDE * (PURE_CHUNK_SIDE / 2));
  ...
  private static void _scanMap(Coord mapOrigin) {
    ...
    final Coord origin = _transformMapCoord(LocalPlayer.position());
  ...
  private static void _reset() {
    ...
    _origin = _transformMapCoord(LocalPlayer.position());
  ...
  private static void _correct() {
    if (_transformMapCoord(LocalPlayer.position()).equals(_currentMoveCoord)) {
      _map[_currentMoveCoord.x][_currentMoveCoord.y] = true;
    } else {
      final int clockwiseOrder = (int)Math.floor((LocalPlayer.a() + Math.PI / 4) / (Math.PI / 2)) % 4;
      final int clockwiseOrderFromSouth = (clockwiseOrder + 3) % 4;
      final Coord previousCoord = LocalPlayer.position()
