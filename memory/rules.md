...
- `designs/`: 여러 decision과 task를 엮은 설계안, 구현 전 구조 초안, 비교적 구체적인 설계 walkthrough를 관리한다.
...
6. 구현 전 구조를 설명하거나 여러 결정을 묶는 설계안이 필요하면 `designs/`에 문서를 만든다.
7. 시도 결과를 바탕으로 방향을 정하면 `decisions/`에 결정을 남긴다.
8. 특정 주제와 관련된 item이 늘어나면 `indexes/`에 주제 인덱스를 만든다.
9. 필요하면 task, decision, design, wiki 문서에서 관련 index를 링크한다.
...
- task, decision, design, wiki 문서는 관련된 index를 링크할 수 있다.
- decision은 관련 issue나 task를 링크할 수 있다.
- design은 관련 decision, task, issue를 링크할 수 있다.
...
- `designs/`는 작업 운영 문서와 설명 문서의 중간에 있는 설계 영역이다.
